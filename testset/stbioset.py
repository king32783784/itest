...
class StbioSet(QDialog):

    def __init__(self, parent=None):
        super(StbioSet, self).__init__(parent)
        self.setWindowTitle(u"I/O稳定性")
        self.resize(450, 550)
    ...
    def createcheckbox(self):
        self.checkbox_io = QtGui.QCheckBox(u'iozone')
        self.connect(self.checkbox_io, QtCore.SIGNAL('clicked()'),
                     self.Oncheckbox_io)

    def createbutton(self):
        self.iosetbutton = QtGui.QPushButton(u"参数设置")
        self.connect(self.iosetbutton, QtCore.SIGNAL('clicked()'),
                     self.Onsetio)
    ...
    def Layout(self):
        ...
        baseLayout.addWidget(self.checkbox_io, 0,0)

        baseLayout.addWidget(self.iosetbutton, 0,3)
    ...
    def initstatus(self):
        testargs = self.readsetting("stressio-user/")
        if testargs["iozonesta"] == "E":
            self.checkbox_io.setChecked(True)
   
    def readsetting(self, setmode):
        self.config = QSettings(SET_FILE, QSettings.IniFormat)
        testargs = {}
        testargs["iozonesta"] = self.config.value(QString(setmode)
                                   + "iozonesta").toString()[0:]
        return testargs

    def updatesetting(self):
        self.config = QSettings(SET_FILE, QSettings.IniFormat)
        self.config.beginGroup("stressio-user")
        for key, value in self.argstemp.iteritems():
            self.config.setValue(key, value)
        self.config.endGroup()

    def Oncheckbox_io(self):
        if self.checkbox_io.isChecked():
            self.argstemp["iozonesta"] = "E"
        else:
            self.argstemp["iozonesta"] = "D"

    def Onsetio(self):
        setiozone = IozonestaSet()
        setiozone.exec_()
       
    def Ondefault(self):
        defaultset = self.readsetting("stressio-default/")
        if defaultset["iozonesta"] == "E":
...
class IozonestaSet(QDialog):
    
    def __init__(self, parent=None):
        super(IozonestaSet, self).__init__(parent)
        self.setWindowTitle("iozone设置")
        self.resize(450,550)
        palette1 = QtGui.QPalette()
        palette1.setColor(self.backgroundRole(), QColor("#cccddc"))
        self.setPalette(palette1)
    ...
    def updatesetting(self):
        self.config = QSettings(SET_FILE, QSettings.IniFormat)
        self.config.beginGroup("iozonesta-user")
        for key, value in self.argstemp.iteritems():
            self.config.setValue(key, value)
        self.config.endGroup()
    ...
    def readsetting(self):
        self.config = QSettings(SET_FILE, QSettings.IniFormat)
        testargs = {}
        testargs["argt"] = self.config.value(QString("iozonesta-user/") + "argt").toInt()[0]
        return testargs
    ...
    def Ondefaultbutton(self):
        self.config = QSettings(SET_FILE, QSettings.IniFormat)
        self.argstemp["argt"] = self.config.value(QString("iozonesta-default/") + "argt").toInt()[0]
